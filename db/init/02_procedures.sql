CREATE OR REPLACE PROCEDURE proc_hire_worker(
...
END;
$$;
CREATE OR REPLACE PROCEDURE proc_hire_itp(
p_full_name VARCHAR(200),
p_birth_date DATE,
p_hire_date DATE,
p_category_code INTEGER,
...
END IF;
IF EXISTS (SELECT 1 FROM worker WHERE employee_id = p_employee_id) THEN
UPDATE worker
SET brigade_id = p_new_brigade_id
WHERE employee_id = p_employee_id;
END IF;
IF EXISTS (SELECT 1 FROM itp WHERE employee_id = p_employee_id) THEN
UPDATE itp
...
END;
$$;
CREATE OR REPLACE PROCEDURE proc_dismiss_employee(
p_employee_id INTEGER,
p_description TEXT DEFAULT NULL
)
LANGUAGE plpgsql AS
...
END;
$$;
CREATE OR REPLACE PROCEDURE proc_assign_brigade_to_stage(
...
END;
$$;
CREATE OR REPLACE PROCEDURE proc_complete_stage(
p_record_id INTEGER,
p_end_date DATE
)
LANGUAGE plpgsql AS
...
END;
$$;
CREATE OR REPLACE PROCEDURE proc_register_test(
p_instance_id INTEGER,
...
END;
$$;
CREATE OR REPLACE PROCEDURE proc_complete_test(
p_test_id INTEGER,
p_result TEXT
)
LANGUAGE plpgsql AS
