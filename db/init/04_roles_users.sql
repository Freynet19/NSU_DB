...
ALTER ROLE role_admin SET lock_timeout = '5s';
ALTER ROLE role_admin SET statement_timeout = '30s';
-- пользователи
CREATE USER user_admin WITH PASSWORD 'admin_password';
CREATE USER user_production WITH PASSWORD 'production_password';
CREATE USER user_hr WITH PASSWORD 'hr_password';
CREATE USER user_workshop WITH PASSWORD 'workshop_password';
CREATE USER user_laboratory WITH PASSWORD 'laboratory_password';
...
GRANT CONNECT ON DATABASE test TO role_admin;
GRANT CONNECT ON DATABASE test TO role_production_report;
GRANT CONNECT ON DATABASE test TO role_hr;
GRANT CONNECT ON DATABASE test TO role_workshop;
GRANT CONNECT ON DATABASE test TO role_laboratory;
GRANT USAGE ON SCHEMA public TO role_admin;
GRANT USAGE ON SCHEMA public TO role_production_report;
GRANT USAGE ON SCHEMA public TO role_hr;
GRANT USAGE ON SCHEMA public TO role_workshop;
GRANT USAGE ON SCHEMA public TO role_laboratory;
