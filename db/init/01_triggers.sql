...
CREATE TRIGGER trg_check_worker_subtype
    ...
    ON worker
    FOR EACH ROW
EXECUTE FUNCTION fn_check_worker_subtype();
CREATE TRIGGER trg_check_itp_subtype
    BEFORE INSERT
    ON itp
    FOR EACH ROW
EXECUTE FUNCTION fn_check_itp_subtype();
CREATE
...
CREATE TRIGGER trg_check_brigadier
    ...
    ON brigade
    FOR EACH ROW
EXECUTE FUNCTION fn_check_brigadier();
CREATE
...
BEGIN
    IF
        NEW.section_head_id IS NOT NULL AND NOT EXISTS (SELECT 1
                                                        FROM itp
                                                        WHERE employee_id = NEW.section_head_id) THEN
...
CREATE TRIGGER trg_check_section_head
    ...
    ON section
    FOR EACH ROW
EXECUTE FUNCTION fn_check_section_head();
CREATE
...
BEGIN
    IF
        NEW.head_id IS NOT NULL AND NOT EXISTS (SELECT 1
                                                FROM itp
                                                WHERE employee_id = NEW.head_id) THEN
...
CREATE TRIGGER trg_check_workshop_head
    ...
    ON workshop
    FOR EACH ROW
EXECUTE FUNCTION fn_check_workshop_head();
CREATE
...
CREATE TRIGGER trg_check_brigade_section
    ...
    ON assembly_record
    FOR EACH ROW
EXECUTE FUNCTION fn_check_brigade_section();
CREATE
...
CREATE TRIGGER trg_check_equipment_lab
    ...
    ON test_equipment
    FOR EACH ROW
EXECUTE FUNCTION fn_check_equipment_lab();
CREATE
...
CREATE TRIGGER trg_check_product_status_for_test
    ...
    ON test
    FOR EACH ROW
EXECUTE FUNCTION fn_check_product_status_for_test();
CREATE
...
CREATE TRIGGER trg_check_product_type_attrs
    ...
    ON product_type
    FOR EACH ROW
EXECUTE FUNCTION fn_check_product_type_attrs();
