CREATE OR REPLACE FUNCTION fn_check_worker_subtype()
RETURNS TRIGGER AS $$
BEGIN
PERFORM employee_id FROM employee
WHERE employee_id = NEW.employee_id
FOR UPDATE;
PERFORM employee_id FROM itp
WHERE employee_id = NEW.employee_id;
IF FOUND THEN
...
END;
$$ LANGUAGE plpgsql;
CREATE OR REPLACE FUNCTION fn_check_itp_subtype()
RETURNS TRIGGER AS $$
BEGIN
PERFORM employee_id FROM employee
WHERE employee_id = NEW.employee_id
FOR UPDATE;
PERFORM employee_id FROM worker
WHERE employee_id = NEW.employee_id;
IF FOUND THEN
...
BEFORE INSERT ON itp
FOR EACH ROW EXECUTE FUNCTION fn_check_itp_subtype();
CREATE OR REPLACE FUNCTION fn_check_brigadier()
RETURNS TRIGGER AS $$
BEGIN
...
CREATE OR REPLACE FUNCTION fn_check_section_head()
RETURNS TRIGGER AS $$
BEGIN
IF NEW.section_head_id IS NOT NULL AND NOT EXISTS (
...
CREATE OR REPLACE FUNCTION fn_check_workshop_head()
RETURNS TRIGGER AS $$
BEGIN
...
CREATE OR REPLACE FUNCTION fn_check_brigade_section()
RETURNS TRIGGER AS $$
DECLARE
v_stage_section_id   INTEGER;
v_brigade_section_id INTEGER;
BEGIN
SELECT section_id INTO v_stage_section_id
FROM production_cycle WHERE stage_id = NEW.stage_id
FOR SHARE;
SELECT section_id INTO v_brigade_section_id
FROM brigade WHERE brigade_id = NEW.brigade_id
FOR SHARE;
IF v_stage_section_id != v_brigade_section_id THEN
...
BEFORE INSERT OR UPDATE ON assembly_record
FOR EACH ROW EXECUTE FUNCTION fn_check_brigade_section();
CREATE OR REPLACE FUNCTION fn_check_equipment_lab()
RETURNS TRIGGER AS $$
DECLARE
v_test_lab_id      INTEGER;
v_equipment_lab_id INTEGER;
BEGIN
SELECT laboratory_id INTO v_test_lab_id
FROM test WHERE test_id = NEW.test_id
FOR SHARE;
SELECT laboratory_id INTO v_equipment_lab_id
FROM equipment WHERE equipment_id = NEW.equipment_id
FOR SHARE;
IF v_test_lab_id != v_equipment_lab_id THEN
...
BEGIN
PERFORM instance_id FROM product_instance
...
IF v_total_stages > 0 AND v_completed_stages >= v_total_stages THEN
UPDATE product_instance
SET status = 'assembled', end_date = CURRENT_DATE
WHERE instance_id = NEW.instance_id
AND status = 'assembling';
ELSIF v_total_stages > 0 AND v_completed_stages < v_total_stages THEN
UPDATE product_instance
...
AFTER INSERT OR UPDATE ON assembly_record
FOR EACH ROW EXECUTE FUNCTION fn_update_product_status();
CREATE OR REPLACE FUNCTION fn_check_product_status_for_test()
RETURNS TRIGGER AS $$
DECLARE
...
WHERE instance_id = NEW.instance_id
FOR UPDATE;
IF v_status NOT IN ('assembled', 'testing') THEN
...
CREATE OR REPLACE FUNCTION fn_check_product_type_attrs()
RETURNS TRIGGER AS $$
DECLARE
v_category_name VARCHAR(100);
BEGIN
SELECT name INTO v_category_name
FROM product_category WHERE category_id = NEW.category_id;
IF v_category_name = 'Автобусы' AND NEW.capacity IS NULL THEN
RAISE EXCEPTION 'Field capacity is required for category Автобусы';
END IF;
IF v_category_name IN ('Сельскохозяйственные машины', 'Дорожно-строительные машины')
AND NEW.productivity IS NULL THEN
RAISE EXCEPTION 'Field productivity is required for category %', v_category_name;
END IF;
IF v_category_name = 'Грузовые автомобили' THEN
IF NEW.load_capacity IS NULL THEN
RAISE EXCEPTION 'Field load_capacity is required for category Грузовые автомобили';
END IF;
IF NEW.engine_type IS NULL THEN
RAISE EXCEPTION 'Field engine_type is required for category Грузовые автомобили';
END IF;
END IF;
IF v_category_name IN ('Легковые автомобили', 'Мотоциклы') AND NEW.engine_type IS NULL THEN
...
BEFORE INSERT OR UPDATE ON product_type
